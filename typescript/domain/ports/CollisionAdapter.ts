...
export class CollisionAdapter implements ICollisionService {
    ...
    public checkCollisions(hitboxData: Float32Array, hitboxCount: number, worldBounds: { x: number, y: number, width: number, height: number }): Promise<Int32Array> {
        return new Promise((resolve) => {
            ...
            this.worker.postMessage({ hitboxData, hitboxCount, worldBounds });
        });
    }
}